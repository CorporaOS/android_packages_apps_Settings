...
public class BluetoothDetailsProfilesController extends BluetoothDetailsController
     ...
     * A2dp profile and Hfp/Headset profile are graied out.
     */
    private void refreshProfilePreference(SwitchPreference profilePref,
        ...
        if (profile instanceof A2dpProfile
                ...
                || HEADSET_CLIENT.equals(profile.toString())) {
            if (isLeAudioEnabled) {
                // If the LeAudio profile is enabled on the LeAudio devices, then the
                // SwitchPreferences of A2dp profile and Hfp/Headset profile are grayed out.
            ...
            if (isLeAudioEnabled && !isBusy) {
                ...
                // If the LeAudio profile is enabled on the LeAudio devices, then the
                // SwitchPreferences of A2dp profile and Hfp/Headset profile are graied out.
                grayOutPreferenceWhenLeAudioIsEnabled(a2dp);
                grayOutPreferenceWhenLeAudioIsEnabled(hfp);
     ...
     * 2) Enable the A2dp profile and Hfp/Headset profile for the associated device. The system
     * can't enable the A2dp profile and Hfp/Headset profile if the Le Audio profile is enabled.
     *
    ...
    private void disableLeAudioProfile(LocalBluetoothProfile profile) {
        ...
        enableProfileAfterUserDisablesLeAudio(hfp);
        enableProfileAfterUserDisablesLeAudio(headset);
    }

    /**
     * When user enable the Le Audio profile, the system needs to do two things.
     * 1) Disable the A2dp profile and Hfp/Headset profile for the associated device. The system
     * can't enable the Le Audio if the A2dp profile and Hfp/Headset profile are enabled.
    ...
    private void enableLeAudioProfile(LocalBluetoothProfile profile) {
        ...
        disableProfileBeforeUserEnablesLeAudio(hfp);

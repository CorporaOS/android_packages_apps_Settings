...
public class SpannableUtil {
    ...
    private static class SpannableFormattable implements Formattable {
        ...
        public void formatTo(Formatter formatter, int flags, int width, int precision) {
            // TODO: Do not ignore flags, width, precision.
            try {
                formatter.out().append(spannable);
            } catch (IOException e) {
    ...
    public static CharSequence titleCaseSentences(Locale locale, CharSequence src) {
        CaseMap.Title titleCasing = CaseMap.toTitle().sentences().noLowercase();
        if (src instanceof Spannable) {
            return applyCaseMapToSpannable(locale, titleCasing, (Spannable) src);
        } else {
            return titleCasing.apply(locale, null, src);
        }
    }
    ...
    private static Spannable applyCaseMapToSpannable(Locale locale, CaseMap.Title caseMap,
        ...
        Log.d("victor", "src:" + src);
        Log.d("victor", "dest:" + dest);
        if (!edits.hasChanges()) {
            return src;
        }

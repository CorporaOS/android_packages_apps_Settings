...
public class ResetNetworkRequest {
     ...
     * @return subscription ID. {@code INVALID_SUBSCRIPTION_ID} means
     ...
     * @return subscription ID. {@code INVALID_SUBSCRIPTION_ID} means resetting APN
     ...
     * @param looper required by ResetNetworkOperationBuilder for calllback support
    ...
    public ResetNetworkOperationBuilder toResetNetworkOperationBuilder(Context context,
        ...
        if ((mResetOptions & RESET_CONNECTIVITY_MANAGER) != 0) {
            builder = builder.resetConnectivityManager();
        }
        if ((mResetOptions & RESET_VPN_MANAGER) != 0) {
            builder = builder.resetVpnManager();
        }
        if ((mResetOptions & RESET_WIFI_MANAGER) != 0) {
            builder = builder.resetWifiManager();
        }
        if ((mResetOptions & RESET_WIFI_P2P_MANAGER) != 0) {
            builder = builder.resetWifiP2pManager(looper);
        }
        if (mResetEsimPackageName != null) {
            builder = builder.resetEsim(mResetEsimPackageName);
        }
        if (mResetTelephonyManager != INVALID_SUBSCRIPTION_ID) {
            builder = builder.resetTelephonyAndNetworkPolicyManager(mResetTelephonyManager);
        }
        if ((mResetOptions & RESET_BLUETOOTH_MANAGER) != 0) {
            builder = builder.resetBluetoothManager();
        }
        if (mResetApn != INVALID_SUBSCRIPTION_ID) {
            builder = builder.resetApn(mResetApn);
        }
        return builder;
    }
}

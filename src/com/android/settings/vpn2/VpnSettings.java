...
public class VpnSettings extends RestrictedDashboardFragment implements
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        if (replaceVpnProfileStore()) {
            ...
            if (mVpnManager.getVpnProfile(FLAG_REPLACE_VPN_PROFILE_STORE) == null) {
                migrateLegacyStore(mVpnManager);
                mVpnManager.putVpnProfile(FLAG_REPLACE_VPN_PROFILE_STORE, new byte[0]);
            }
        }
    }
    ...
    private static List<VpnProfile> loadVpnProfiles(VpnManager vpnManager) {
        ...
        if (replaceVpnProfileStore()) {
            vpnKeys = vpnManager.listVpnProfile(Credentials.VPN);
        } else {
        ...
        for (String key : vpnKeys) {
            ...
            if (replaceVpnProfileStore()) {
                profile = VpnProfile.decode(key, vpnManager.getVpnProfile(Credentials.VPN + key));
            } else {
    ...
    private void migrateLegacyStore(VpnManager vpnManager) {
        ...
        for (String prefix : prefixes) {
            for (String key : LegacyVpnProfileStore.list(prefix)) {
                ...
                if (!vpnManager.putVpnProfile(name, LegacyVpnProfileStore.get(name))) {

...
public class SnoopLoggerFiltersPreferenceController extends BasePreferenceController {
    ...
    public SnoopLoggerFiltersPreferenceController(Context context, String key) {
        ...
        mListValues = context.getResources().getStringArray(
                R.array.bt_hci_snoop_log_filters_values);
        mListEntries = context.getResources().getStringArray(
                R.array.bt_hci_snoop_log_filters_entries);
    }

    @Override
    public int getAvailabilityStatus() {
        var snoopLogMode = BluetoothProperties.snoop_log_mode()
    ...
    public void displayPreference(PreferenceScreen screen) {
        ...
        for (int i = 0; i < mListValues.length; i++) {
            mGroup.addPreference(new SnoopLoggerFiltersPreference(
                    prefContext, mListValues[i], mListEntries[i]));
        }
    }
}

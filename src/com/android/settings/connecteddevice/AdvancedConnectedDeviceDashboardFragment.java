...
public class AdvancedConnectedDeviceDashboardFragment extends DashboardFragment {
    ...
    public void onAttach(Context context) {
        ...
        ThreadNetworkPreferenceController threadController = use(ThreadNetworkPreferenceController.class);
        // We only need the observer listen to the broadcast in the background for refreshing
        // UI if the device supports Thread.
        if (threadController != null && threadController.isThreadSupportedOnDevice()) {
            if (getSettingsLifecycle() != null) {
                getSettingsLifecycle().addObserver(threadController);
            }
        }
    }

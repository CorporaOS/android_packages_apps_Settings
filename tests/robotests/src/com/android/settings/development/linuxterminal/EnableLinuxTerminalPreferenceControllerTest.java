...
public class EnableLinuxTerminalPreferenceControllerTest {
    ...
    public void setup() throws Exception {
        ...
        doReturn(mInstalled ? mApplicationInfo : null)
                .when(mPackageManager)
                .getApplicationInfo(eq(mTerminalPackageName), any());
        doReturn(
                        mEnabled
        ...
        mController =
                Mockito.spy(
                        new EnableLinuxTerminalPreferenceController(
                                mContext, mUserContext, /* userId= */ 0));
        doReturn(mInstalled ? mTerminalPackageName : null)
                .when(mController)
                .getTerminalPackageName();
        doReturn(mIsPrimaryUser).when(mController).isPrimaryUser();

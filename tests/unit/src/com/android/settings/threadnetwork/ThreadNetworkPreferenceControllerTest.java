...
import com.android.net.thread.platform.flags.FakeFeatureFlagsImpl;
...
public final class ThreadNetworkPreferenceControllerTest {
    ...
    private FakeFeatureFlagsImpl mFakeFeatureFlagsImpl;

    @Before
    public void setUp() throws Exception {
        mFakeFeatureFlagsImpl = new FakeFeatureFlagsImpl();
        mFakeFeatureFlagsImpl.setFlag(Flags.FLAG_THREAD_ENABLED_PLATFORM, true);
    ...
    @Test
    public void getAvailabilityStatus_flagDisabled_returnsUnsupportedOnDevice() throws Exception {
        mFakeFeatureFlagsImpl.setFlag(Flags.FLAG_THREAD_ENABLED_PLATFORM, false);
    ...
    public void isChecked_threadSetEnabled_returnsTrue() {
        mFakeThreadNetworkController.setEnabled(true, mExecutor, unused -> {});
        mController.onStart();
    ...
    public void isChecked_threadSetDisabled_returnsFalse() {
        mFakeThreadNetworkController.setEnabled(false, mExecutor, unused -> {});
        mController.onStart();
    ...
    private static final class FakeThreadNetworkController implements BaseThreadNetworkController {
        ...
        public void setEnabled(
            ...
            if (mStateCallback != null) {
                if (!mIsEnabled) {
                    mExecutor.execute(
                            () -> mStateCallback.onThreadEnableStateChanged(STATE_DISABLING));
                    mExecutor.execute(
                            () -> mStateCallback.onThreadEnableStateChanged(STATE_DISABLED));
                } else {
                    mExecutor.execute(
                            () -> mStateCallback.onThreadEnableStateChanged(STATE_ENABLED));
                }
            }
        ...
        public void registerStateCallback(Executor executor, StateCallback callback) {
            ...
            int enabledState = mIsEnabled ? STATE_ENABLED : STATE_DISABLED;
            mExecutor.execute(() -> mStateCallback.onThreadEnableStateChanged(enabledState));
        }
